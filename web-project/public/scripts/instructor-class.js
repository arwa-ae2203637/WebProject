    ...
    try {
      ...
      const loggedUser = dh.getLoggedUser(users);
       students = users.filter(user => {
        return user.userType === "student" && user.enrollments.some(course => course.crn === selectedClass.crn && course.status !== "pending")
    });
...
function updateStudentTable(students, selectedClassCRN) {
    ...
    studentTable.innerHTML = students.length > 0 
        ? students.map(student => {
            const course = student.enrollments.find(c => c.crn === selectedClassCRN);
        ...
        dropdown.addEventListener('change', async (e) => {
            ...
            if (confirm(`Change grade from ${initialGrade} to ${newGrade}?`)) {
                try {
                    ...
                    const courseToUpdate = studentToUpdate.enrollments.find(c => c.crn === crn);

import { getAllClasses , getAllUsers, getLoggedUser, updateUserProfile } from "./data-handling.js";

document.addEventListener("DOMContentLoaded", async () => {
    try {
        const selectedCourse = JSON.parse(localStorage.getItem("selectedCourse"));
        const courseClasses = await getAllClasses(selectedCourse.id);
...
async function enrollInClass(crn,course_id) {
  try {
      ...
      const hasPending = student.pendingCourses.some(c => c.crn === crn);
      const hasEnrolled = student.completedCourses?.some(c => c.crn === crn);
